...
(function ($, window, undefined) {
    var _ie6 = window.ActiveXObject && !window.XMLHttpRequest, _fn = function () { }, _count = 0, _rurl = /^url:/, _singleton, onKeyDown, document = window.document, expando = "JDG" + +new Date(),
dialogTpl =
 ...
 */
    ...
    _skin = _getArgs("skin") || "default", /*! 获取 lhgdialog 可跨级调用的最高层的 window 对象和 document 对象 */
    _doc, _top = function (w) {
        ...
        if (_getArgs("self") === "true" || _doc.getElementsByTagName("frameset").length > 0) {
            _doc = w.document;
            return w;
        }
        return w["top"];
    } (window), _root = _doc.documentElement, _doctype = _doc.compatMode === "BackCompat";
    var _$doc = $(_doc), _$top = $(_top), _$html = $(_doc.getElementsByTagName("html")[0]);
    /*! 开启IE6 CSS背景图片缓存 */
    try {
    ...
    (function (style) {
        if (!style) {
            var head = _doc.getElementsByTagName("head")[0], link = _doc.createElement("link");
    ...
    */
    ...
    lhgdialog.fn = lhgdialog.prototype = {
        ...
        _init: function (config) {
            var that = this, DOM, content = config.content, isIfr = _rurl.test(content);
            that.opener = window;
            that.config = config;
            ...
            //当cancel参数为false时隐藏关闭按钮
            DOM.content[0].style.padding = config.padding;
            that.button.apply(that, config.button);
            that.title(config.title).content(content, true, isIfr).size(config.width, config.height).position(config.left, config.top).time(config.time)[config.show ? "show" : "hide"](true).zindex();
            config.focus && that.focus();
            config.lock && that.lock();
            config.lockScroll && that.lockScroll();
        ...
        */
        button: function () {
            var that = this, DOM = that.DOM, buttons = DOM.buttons[0], focusButton = "ui_state_highlight", listeners = that._listeners = that._listeners || {}, ags = [].slice.call(arguments), i = 0, item, value, id, isNewButton, button;
            for (; i < ags.length; i++) {
                item = ags[i];
                value = item.name;
                id = item.id || value;
        ...
        */
        title: function (text) {
            ...
            var DOM = this.DOM, border = DOM.border, title = DOM.title[0];
            if (text === false) {
        ...
        */
        content: function (msg, add, frm) {
            ...
            var that = this, DOM = that.DOM, wrap = DOM.wrap[0], width = wrap.offsetWidth, height = wrap.offsetHeight, left = parseInt(wrap.style.left), top = parseInt(wrap.style.top), cssWidth = wrap.style.width, $content = DOM.content, loading = lhgdialog.setting.content;
        ...
        */
        size: function (width, height) {
            var that = this, DOM = that.DOM, wrap = DOM.wrap[0], style = DOM.main[0].style;
        ...
        */
        position: function (left, top) {
            var that = this, config = that.config, wrap = that.DOM.wrap[0], style = wrap.style, isFixed = _ie6 ? false : config.fixed, ie6Fixed = _ie6 && config.fixed, docLeft = _$top.scrollLeft(), docTop = _$top.scrollTop(), dl = isFixed ? 0 : docLeft, dt = isFixed ? 0 : docTop, ww = _$top.width(), wh = _$top.height(), ow = wrap.offsetWidth, oh = wrap.offsetHeight;
            if (left || left === 0) {
        ...
        */
        time: function (second, callback) {
            var that = this, timer = that._timer;
        ...
        /*! 置顶对话框 */
        zindex: function () {
            var that = this, DOM = that.DOM, load = that._load, top = lhgdialog.focus, index = lhgdialog.setting.zIndex++;
            // 设置叠加高度
        ...
        /*! 锁屏 */
        lock: function () {
            var that = this, frm, index = lhgdialog.setting.zIndex - 1, config = that.config, mask = $("#ldg_lockmask", _doc)[0], style = mask ? mask.style : "", positionType = _ie6 ? "absolute" : "fixed";
            if (!mask) {
        ...
        /*! 解除锁屏 */
        unlock: function () {
            var that = this, config = that.config, mask = $("#ldg_lockmask", _doc)[0];
            if (mask && that._lock) {
                // 无限级锁屏
        ...
        unlockScroll: function () {
            ...
            var topDoc = topWindow().document;
            var topWinBody = topDoc.getElementsByTagName(topDoc.compatMode == "BackCompat" ? "BODY" : "HTML")[0];
            if (topWinBody.styleOverflow != undefined) if (window.navigator.userAgent.indexOf("Firefox/3.6") != -1) {
                var topWinBodyScrollTop = topWinBody.scrollTop;
        ...
        /*! 关闭对话框 */
        close: function () {
            var that = this, DOM = that.DOM, wrap = DOM.wrap, list = lhgdialog.list, fn = that.config.close;
            that.time();
            ...
            // 移除HTMLElement或重用
            if (_singleton) wrap.remove(); else {
                _singleton = that;
                if (that._minState) {
        ...
        /*! 最大化窗口 */
        max: function () {
            var that = this, maxSize, DOM = that.DOM, wrapStyle = DOM.wrap[0].style, mainStyle = DOM.main[0].style, rbStyle = DOM.rb[0].style, titleStyle = DOM.title[0].style, config = that.config, top = _$top.scrollTop(), left = _$top.scrollLeft();
            if (!that._maxState) {
        ...
        /*! 最小化窗口 */
        min: function () {
            var that = this, DOM = that.DOM, main = DOM.main[0].style, buttons = DOM.buttons[0].style, dialog = DOM.dialog[0].style, rb = DOM.rb[0].style.cursor, resize = that.config.resize;
            if (!that._minState) {
        ...
        */
        get: function (id, object) {
            if (lhgdialog.list[id]) {
                if (object === 1) return lhgdialog.list[id]; else return lhgdialog.list[id].content || null;
            }
            return null;
        },
        /**
        ...
        */
        _iframe: function (url) {
            var that = this, iframe, $iframe, iwin, $idoc, $ibody, iWidth, iHeight, $content = that.DOM.content, config = that.config, loading = that._load = $(".ui_loading", $content[0])[0], initCss = "position:absolute;left:-9999em;border:none 0;background:transparent", loadCss = "width:100%;height:100%;border:none 0;";
            // 是否允许缓存. 默认true
            ...
            var load = that._fmLoad = function () {
                ...
                var DOM = that.DOM, ltSize, lt = DOM.lt[0].offsetHeight, main = DOM.main[0].style;
                ...
                try {
                    iwin = that.content = iframe.contentWindow;
                    // 定义窗口对象content属性为内容页的window对象
                    $idoc = $(iwin.document);
        ...
        /*! 获取窗口元素 */
        _getDOM: function () {
            var wrap = _doc.createElement("div"), body = _doc.body;
        ...
        */
        ...
        /*! 计算最大化窗口时窗口的尺寸 */
        _maxSize: function () {
            var that = this, DOM = that.DOM, wrap = DOM.wrap[0], main = DOM.main[0], maxWidth, maxHeight;
        ...
        _ie6PngFix: function () {
            if (_ie6) {
                var i = 0, elem, png, pngPath, runtimeStyle, path = lhgdialog.setting.path + "/skins/", list = this.DOM.wrap[0].getElementsByTagName("*");
                for (; i < list.length; i++) {
                    elem = list[i];
                    ...
                    if (png) {
                        ...
                        runtimeStyle.filter = "progid:DXImageTransform.Microsoft." + "AlphaImageLoader(src='" + pngPath + "',sizingMethod='scale')";
                    }
                }
            }
            return this;
        ...
        _ie6SelectFix: _ie6 ? function () {
            var $wrap = this.DOM.wrap, wrap = $wrap[0], expando = expando + "iframeMask", iframe = $wrap[expando], width = wrap.offsetWidth, height = wrap.offsetHeight;
            width = width + "px";
            height = height + "px";
            if (iframe) {
        ...
        _setFixed: function (el) {
            ...
            if (_ie6) {
                var sLeft = _$top.scrollLeft(), sTop = _$top.scrollTop(), left = parseInt(style.left) - sLeft, top = parseInt(style.top) - sTop, txt = _doctype ? "this.ownerDocument.body" : "this.ownerDocument.documentElement";
                this._setAbsolute();
        ...
        /*! 重置位置与尺寸 */
        _reset: function () {
            var test = !!window.ActiveXObject, newSize, that = this, tw = _$top.width(), tt = _$top.height(), oldSize = that._winSize || tw * tt, oldWidth = that._lockDocW || tw, left = that._left, top = that._top;
            if (test) {
                //IE6下遮罩大小改变
            ...
            if (left || top) that.position(left, top);
        },
        _addEvent: function () {
            var resizeTimer, that = this, config = that.config, DOM = that.DOM;
            // 窗口调节事件
            ...
            // 双击标题栏最大化还窗口事件
            if (config.max) DOM.title.bind("dblclick", function () {
                that.max();
                return false;
            });
        },
    ...
    */
    onKeyDown = function (event) {
        var target = event.target, api = lhgdialog.focus, keyCode = event.keyCode;
    ...
    *------------------------------------------------
    */
    var _use, _isSetCapture = "setCapture" in _root, _isLosecapture = "onlosecapture" in _root;
    lhgdialog.dragEvent = {
        onstart: _fn,
    ...
    _use = function (event) {
        var limit, startWidth, startHeight, startLeft, startTop, isResize, api = lhgdialog.focus, config = api.config, DOM = api.DOM, wrap = DOM.wrap[0], title = DOM.title, main = DOM.main[0], _dragEvent = lhgdialog.dragEvent, // 清除文本选择
        clsSelect = "getSelection" in _top ? function () {
        ...
        _dragEvent.onmove = function (x, y) {
            if (isResize) {
                var wrapStyle = wrap.style, style = main.style, width = x + startWidth, height = y + startHeight;
        ...
        limit = function (fixed) {
            var ow = wrap.offsetWidth, // 向下拖动时不能将标题栏拖出可视区域
            oh = title[0].offsetHeight || 20, ww = _$top.width(), wh = _$top.height(), dl = fixed ? 0 : _$top.scrollLeft(), dt = fixed ? 0 : _$top.scrollTop();
            // 坐标最大值限制(在可视区域内)	
            maxX = ww - ow + dl;
            maxY = wh - oh + dt;
            return {
    ...
    */
    $(function () {
        ...
        lhgdialog.setting.extendDrag && function (dragEvent) {
            var mask = _doc.createElement("div"), style = mask.style, positionType = _ie6 ? "absolute" : "fixed";
            ...
            dragEvent.start = function () {
                var api = lhgdialog.focus, main = api.DOM.main[0], iframe = api.iframe;
